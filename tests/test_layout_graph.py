...
class TestLayoutGraph(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        space_hulk_file.write_text(cls._SPACE_HULK.json(exclude_none=True,
                                                        indent=2))
        cls._LAYOUT.save(directory=cls.OUTPUT_FOLDER,
                         filename="space_hulk_layout.dot")

    def setUp(self) -> None:
        self.space_hulk = copy(self._SPACE_HULK)
        self.layout = copy(self._LAYOUT)

    def test_number_properties_on_all_rooms_hulk_expect_correct_numbers_returned(
            self) -> None:
    ...
    def test_str_with_same_comment_on_multiple_calls_expect_comment_unchanged_after_each_call(
            self) -> None:
    ...
    def test_layout_edge_type_property_access_expect_set_type_returned(
            self) -> None:
    ...
    def test_layout_engine_property_access_expect_set_engine_returned(
            self) -> None:
    ...
    def test_render_pdf_expect_no_errors(self) -> None:
        self.layout.render_pdf(file_name=self.OUTPUT_FOLDER / f"space_hulk.pdf",
                               space_hulk=self.space_hulk)
