...
class TestSpaceHulkTables(unittest.TestCase):
    ...
    def test_getitem_with_all_valid_table_names_expect_always_correct_table_returned(self) -> None:
        for table_name, table_value in self.space_hulk:
            with self.subTest(i=table_name):
                assert self.space_hulk[table_name] == table_value

    def test_getitem_with_invalid_table_name_expect_error(self) -> None:
    ...
    def test_setitem_with_empty_value_for_table_that_prohibits_empty_values_expect_error(self) -> None:
        with pytest.raises(EventCountOutOfRangeError):
            self.space_hulk["rooms"].events = []

    # noinspection PyTypeChecker
    def test_setitem_with_invalid_value_type_on_table_with_previously_set_empty_value_expect_error(self) -> None:
        with self.subTest(i="Set empty value on table that allows empty values"):
            self.space_hulk["hazards"].events = []
        with self.subTest(i="Set invalid value"), pytest.raises(EventTypeError):
            self.space_hulk["hazards"].events = [5]

    # noinspection PyTypeChecker
    def test_setitem_with_invalid_value_types_expect_always_error(self) -> None:
        for table_name, table_value in self.space_hulk:
            with self.subTest(i=table_name):
    ...
    def test_setitem_with_invalid_table_name_expect_error(self) -> None:
        with pytest.raises(AttributeError):
    ...
    def test_markdown_expect_same_as_stored_markdown_file(self) -> None:
        ...
        assert expected_markdown == self.space_hulk.as_markdown()
